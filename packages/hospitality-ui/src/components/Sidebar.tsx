...
function SidebarSection({ title, links }: SidebarSectionType) {
  ...
  return (
    <li className="flex flex-col gap-y-2 border-y border-gray-500 first:pt-4 last:pb-4">
      <h3
      ...
      <div
        ...
        {links.map((link) => (
          <Link
            key={link.to}
            className="h-fit px-4 py-2 font-semibold duration-0 select-none hover:bg-white hover:text-gray-900"
            to={link.to}
            variant="secondary">
            {link.title}
          </Link>
