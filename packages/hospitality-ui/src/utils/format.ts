...
export function formatISOToString(value: string | null) {
  if (!value) return "";
...
export function getDayCountString(days: number) {
  ...
  return `${Math.abs(Math.ceil(days))} day(s)${days < 0 ? " ago" : ""}`;
}

const rsdCurrencyFormatter = new Intl.NumberFormat("rs-RS", {
  style: "currency",
  currency: "RSD",
